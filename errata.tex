...
$\ket{\Psi_{0}}$ also serves an example that a product state may
<<<<<<< HEAD
have $\overline{P_{\fh}}=0$ because of modular arithmetic. In the
continuous case, $\overline{P_{\fh}}=0$ is equivalent to 
=======
have $\overline{P_{\fh}}=0$ because of modular arithmetic. To minimized
the modular arithmetic effect, we define the maximally entangled states
by
>>>>>>> branch 'master' of https://github.com/yuttai/Erratum-Geometry-of-discrete-quantum-computing.git
\begin{equation}
\forall j,\forall\mu,\braket{\sigma_{\mu}^{j}}=0\label{eq:john}
\end{equation}
<<<<<<< HEAD
Eq. (\ref{eq:john}) defines maximally entangled states over $\ff{p^{2}}$
as well. In this definition, the Bell states are maximally entangled
while a normalized state $\ket{\Psi_{2}}$ over $\ff{3^{2}}$:
\begin{eqnarray}
\ket{\Psi_{2}} & = & \ket{01}+\ket{10}+\left(-1-\ComplexI\right)\ket{11}
\end{eqnarray}
is not maximally entangled although $\overline{P_{\fh}^{j}}=0$ for
all $j$.
=======
For example, consider a normalized state $\ket{\Psi_{2}}$ over $\ff{3^{2}}$:
\begin{eqnarray}
\ket{\Psi_{2}} & = & \ket{01}+\ket{10}+\left(-1-\ComplexI\right)\ket{11}
\end{eqnarray}
Although $\overline{P_{\fh}^{j}}=0$ for all $j$, $\ket{\Psi_{2}}$
does not satisfy Eq. (\ref{eq:john}). Therefore, $\ket{\Psi_{2}}$
